...
def Build_LeViT_UNet_384(num_classes=4, distillation=True,
              pretrained=False, fuse=False, img_size=224):
    return model_factory_v3_NM(**specification['LeViT_384'], num_classes=num_classes,
                         distillation=distillation, pretrained=pretrained, fuse=fuse, img_size=img_size)
...
def model_factory_v3_NM(C, D, X, N, drop_path, weights,
                  num_classes, distillation, pretrained, fuse, img_size):
...
if __name__ == '__main__':
    net = LeViT_UNet_384()
    net.eval()
    net(torch.randn(20, 1, 224, 224))
    #print(net.FLOPS, 'FLOPs', sum(p.numel() for p in net.parameters() if p.requires_grad), 'parameters')
